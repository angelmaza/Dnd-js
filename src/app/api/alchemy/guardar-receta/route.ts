...
export async function POST(req: Request) {
  const rec: RecetasViewFinal = await req.json().catch(() => null as any);
  if (!rec || !rec.NombreProducto?.trim() || !Array.isArray(rec.Elementos) || rec.Elementos.length === 0) {
  ...
  const rProd = await execute(
    ...
    [rec.NombreProducto.trim(), rec.Descripcion ?? null, 1]
  );
  const idProducto = rProd.insertId;

  // 2) Insertar relaciones (solo válidas)
  for (const item of rec.Elementos) {
